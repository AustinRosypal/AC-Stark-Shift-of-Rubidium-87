...
def find_good_row_triples(
    ...
    """
    Find triples of rows where:
        a1 + a2 + a3 (scalar) is close to 0
        c1 + c2 + c3 (tensor) is close to 0
        b1 + b2 + b3 (vector) is large

    Acceptance conditions:
        abs(b1 + b2 + b3) > ratio_threshold * abs(a1 + a2 + a3)
        abs(b1 + b2 + b3) > ratio_threshold * abs(c1 + c2 + c3)

    By default, also require:
        b1 + b2 + b3 > 0

    Optional geometric constraint:
        sqrt((a1+a2+a3)^2 + (c1+c2+c3)^2) <= max_ac_radius

    Optional component-wise constraints:
        abs(a1+a2+a3) <= max_abs_a_sum
        abs(c1+c2+c3) <= max_abs_c_sum
    ...
    center_wavelength = 1529.314
    eom_width = 0.3904
    df = df[(df[wavelength_col] >= center_wavelength - eom_width) & (df[wavelength_col] <= center_wavelength + eom_width)].copy()

    if len(df) < 3:
        raise ValueError(f"Fewer than three rows found with {mF_col} = {mF_value}.")
    ...
    if n < 3:
        raise ValueError("CSV must contain at least three rows.")

    # We search in the (a, c) plane because we want a_k ≈ -(a_i + a_j) and c_k ≈ -(c_i + c_j)
    ...
    if max_ac_radius is None:
        ...
        max_ac_radius = 1e-3*data_scale #0.05 * data_scale
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            target = np.array([-(a[i] + a[j]), -(c[i] + c[j])])
            ...
            for k in candidate_indices:
                # Avoid self-pairing and duplicate triples.
                if k <= j:
                    continue

                # EOM Capability Cut
                condition_EOM_k = wavelength[k] >= 1529.314 - 0.3904 and wavelength[k] <= 1529.314 + 0.3904
                if not condition_EOM_k:
                    continue
                ...
                #     |b_sum| > R |a_sum|
                #     |b_sum| > R |c_sum|
                #     Each tone lies within the EOM-accessible window.
                ...
                record = {
                    ...
                    "Tone3 Wavelength (nm)": wavelength[k],
                    "Combined_ST_Magnitude": ac_error,

                    # For polarizability ratios
                    "alphaS_1": a[i],
                    "alphaV_1": b[i],
                    "alphaT_1": c[i],
                    "alphaS_2": a[j],
    ...
    if len(results) == 0:
        print("No valid row triples found.")
        return results

    # Sort first by the two cancellation ratios, then by large vector shift.
    if require_positive_b_sum:
        results = results.sort_values(
            by=["alphaV to alphaT Ratio", "alphaV to alphaS Ratio", "alphaV_sum", "Combined_ST_Magnitude"],
            ascending=[False, False, False, True]
        )
    else:
        results = results.sort_values(
            by=["alphaV to alphaT Ratio", "alphaV to alphaS Ratio", "abs_alphaV_sum", "Combined_ST_Magnitude"],
            ascending=[False, False, False, True]
        )
    ...
    print(f"Found {len(results)} valid row triples.")
...
if __name__ == "__main__":
    ...
    results = find_good_row_triples(
        csv_path="DataROI_perIntensity.csv",
        output_path="ThreeFourierTones_alphas.csv",
        ...
        # |b1 + b2 + b3| > 10*|a1 + a2 + a3|
        # |b1 + b2 + b3| > 10*|c1 + c2 + c3|
        ratio_scalar_threshold=100.0,
        ratio_tensor_threshold=100.0,
        ...
        # True means b1 + b2 + b3 must be positive.
