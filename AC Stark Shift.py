...
if (analyze_5p32):
    ...
    energy_final_raw = [0, 19355.203, 25703.498, 28689.390, 30281.620, 31222.453]  # 5p_3/2 analysis
    RME_d_raw = [-6.003, -10.889, 1.80, 1.658, -1.118, -0.855] # 5p_3/2 analysis
    ...
    m_F = 2         # Zeeman sublevels of F, projection of F along the quantization axis, if you will.
    Fprime = 3   # For matrix elements
    w = 2.414192e15
    nu = w / 2 / pi   # Since w = 2*pi*nu and it's good to express frequency as nu
    energy_initial = 0.
    energy_final_raw = [12816.545, 23792.591] # 5s_1/2 analysis
    RME_d_raw = [6.003, 0.5230]  # 5s_1/2 analysis
...
energy_final = [0]*len(energy_final_raw)
for i,value in enumerate(energy_final_raw):
    energy_final[i] = energy_final_raw[i]*1.98645e-23
RME_d = [0]*len(RME_d_raw)
for i,value in enumerate(RME_d_raw):
    RME_d[i] = RME_d_raw[i]*eCharge*a0
scalar_val_arr = [0]*len(energy_final)
vector_val_arr = [0]*len(energy_final)
tensor_val_arr = [0]*len(energy_final)
delta_E_arr = [0]*len(energy_final)
contrib_arr = [0]*len(energy_final)
scalar_val_arr_ind = [0]*len(energy_final)
vector_val_arr_ind = [0]*len(energy_final)
tensor_val_arr_ind = [0]*len(energy_final)
delta_E_arr_ind = [0]*len(energy_final)
...
def scalar(F, Fprime, w):
    scalar_temp_val = 0.0
    for i,value in enumerate(energy_final):
        if i == 0: Fprime = 2 # 5s_1/2 contribution
        else: Fprime = 4
        omega_FprimeF = 1/hbar*(energy_final[i] - energy_initial)
        scalar_temp_val += ((2/3/hbar)*omega_FprimeF/(omega_FprimeF**2 - w**2) * RME_d[i]**2 )
        scalar_val_arr[i] = scalar_temp_val
        scalar_val_arr_ind[i] = ((2/3/hbar)*omega_FprimeF/(omega_FprimeF**2 - w**2) * RME_d[i]**2 )
...
def vector(F, Fprime, w):
    ...
    for i, value in enumerate(energy_final):
        if i == 0: Fprime = 2 # 5s_1/2 contribution
        omega_FprimeF = 1/hbar*(energy_final[i] - energy_initial)
        vector_temp_val += ( (-1)**(F+Fprime+1) * math.sqrt(6.0*F*(2*F+1)/(F+1)) * w6j / hbar * omega_FprimeF/(omega_FprimeF**2 - w**2) * RME_d[i]**2 )
        vector_val_arr[i] = vector_temp_val
        vector_val_arr_ind[i] = ( (-1)**(F+Fprime+1) * math.sqrt(6.0*F*(2*F+1)/(F+1)) * w6j / hbar * omega_FprimeF/(omega_FprimeF**2 - w**2) * RME_d[i]**2 )
...
def tensor(F, Fprime, w):
    ...
    for i, value in enumerate(energy_final):
        if i == 0: Fprime = 2 # 5s_1/2 contribution
        omega_FprimeF = 1/hbar*(energy_final[i] - energy_initial)
        tensor_temp_val += ( (-1)**(F+Fprime) * math.sqrt(40.0*F*(2*F+1)*(2*F-1)/3/(F+1)/(2*F+3)) * w6j / hbar * omega_FprimeF/(omega_FprimeF**2 - w**2) * RME_d[i]**2 )
        tensor_val_arr[i] = tensor_temp_val
        tensor_val_arr_ind[i] = ( (-1)**(F+Fprime) * math.sqrt(40.0*F*(2*F+1)*(2*F-1)/3/(F+1)/(2*F+3)) * w6j / hbar * omega_FprimeF/(omega_FprimeF**2 - w**2) * RME_d[i]**2 )
...
for lambda_ in np.arange(0, Npt) + min:
    real_lambda = (max - min)/(Npt)*(lambda_-min) + min
    ...
    arr_scalarTerm[lambda_-min] = energy_shift(alpha_0, 0, 0)
    arr_vectorTerm[lambda_-min] = energy_shift(0, alpha_1, 0)
    arr_tensorTerm[lambda_-min] = energy_shift(0, 0, alpha_2)
    arr_wavelength[lambda_-min] = real_lambda
    arr_alpha0[lambda_-min] = alpha_0
    arr_alpha1[lambda_-min] = alpha_1
    arr_alpha2[lambda_-min] = alpha_2
    arr_eShift[lambda_-min] = delta_E
    ...
    if (real_lambda == lambda_of_interest):
        for i, value in enumerate(scalar_val_arr):
            contrib_arr[i] = i + 1
            delta_E_arr[i] = energy_shift(scalar_val_arr[i], vector_val_arr[i], tensor_val_arr[i])
            delta_E_arr_ind[i] = energy_shift(scalar_val_arr_ind[i], vector_val_arr_ind[i], tensor_val_arr_ind[i])
