...
# --------------------------------------------------------------------
# Calculate the shift of the 5P_3/2 F = 3 State
F_qn = 3        # Total hyperfine angular momentum quantum number (F=J+I)
J_qn = 1.5
w6j_vector = np.array( [ float(wigner_6j(1, 1, 1, F_qn, F_qn, 2)), float(wigner_6j(1, 1, 1, F_qn, F_qn, 3)), float(wigner_6j(1, 1, 1, F_qn, F_qn, 4)) ] )
w6j_tensor = np.array( [ float(wigner_6j(1, 1, 2, F_qn, F_qn, 2)), float(wigner_6j(1, 1, 2, F_qn, F_qn, 3)), float(wigner_6j(1, 1, 2, F_qn, F_qn, 4)) ] )
energy_initial = 12816.551462 * h_planck * c * 100 # E_nJF , Look up this value (using NIST Atomic Spectra Database) and report it in units of cm^-1 - converted to Joules here
...
[np.nan, 19355.203073, 25703.520038, 28689.390017, 30281.620010, 31222.453006,
    19355.650825, 25700.537667, 28687.127459, 30280.113283, 31221.440377],
[np.nan, 19355.200929, 25703.519080, 28689.389580, 30281.619753, 31222.452852,
    np.nan, np.nan, np.nan, np.nan, np.nan] ] ) * h_planck * c * 100 # units of Joules

# Reduced Matrix Elements calculated by ARC
#RME_elec = np.array( [5.978, 10.634000000000002, 2.3340000000000005, 1.6580000000000001, -1.0766230094946644, 0.8275587122101007] ) * (eCharge*a0) # 5p_3/2 analysis
    ...
    for i, row in enumerate(energies_hf): # 0, 1, 2
        for j, energy_hf in enumerate(row):  # 0, 1, 2, 3, 4, 5
            ...
            if i == 0: 
                Fprime = 2
                if j == 0: Jprime = 0.5
                elif (j > 0 and j < 6): Jprime = 2.5
                elif (j > 5): Jprime = 1.5
            elif i == 1: 
                Fprime = 3
                if (j > 0 and j < 6): Jprime = 2.5
                elif (j > 5): Jprime = 1.5
            elif i == 2:
                Fprime = 4
                if (j > 0 and j < 6): Jprime = 2.5
...
df.to_csv("June10_5p_32_Data.csv", index=False)
